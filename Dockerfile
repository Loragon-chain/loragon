...
# then log output of file /code/bin/loragond
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/loragond \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/loragond | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.21

COPY --from=build-env /code/build/loragond /usr/bin/loragond
