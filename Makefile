...
HTTPS_GIT := github.com/loragon-chain/loragon.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=loragon \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=loragond \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/loragond ./cmd/loragond
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/loragond.exe ./cmd/loragond
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/loragond
...
draw-deps:
	...
	@goviz -i ./cmd/loragond -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c loragon --local -f chains.yaml
endif
...
set-testnet-configs:
	loragond config set client chain-id localchain_9000-1
	loragond config set client keyring-backend test
	loragond config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | loragond keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | loragond keys add acc1 --recover`
